...
#include <string.h>
  ...
  int handle_option(char *arg) {
    if (!strncmp(arg, "--icache=", 9)) {
      sscanf(arg + 9, "%u:%u:%u:%u", &icacheSets, &icacheAssoc,
             &icacheBlocksize, &icacheHitTime);
    } else if (!strncmp(arg, "--dcache=", 9)) {
      sscanf(arg + 9, "%u:%u:%u:%u", &dcacheSets, &dcacheAssoc,
             &dcacheBlocksize, &dcacheHitTime);
    } else if (!strncmp(arg, "--l2cache=", 10)) {
      sscanf(arg + 10, "%u:%u:%u:%u", &l2cacheSets, &l2cacheAssoc,
             &l2cacheBlocksize, &l2cacheHitTime);
    ...
    if (icacheSets) {
      ...
      if (icacheRefs > 0) {
        ...
        printf("  avg I-cache access time: %13.2f cycles\n",
               (double)((icachePenalties + icacheRefs * icacheHitTime)) /
                   icacheRefs);
      } else {
    ...
    if (dcacheSets) {
      ...
      if (dcacheRefs > 0) {
        ...
        printf("  avg D-cache access time: %13.2f cycles\n",
               (double)((dcachePenalties + dcacheRefs * dcacheHitTime)) /
                   dcacheRefs);
      } else {
  ...
  // Returns True if Successful
  //
  int read_mem_access(uint32_t * pc, uint32_t * addr, char *i_or_d,
                      char *r_or_w) {
